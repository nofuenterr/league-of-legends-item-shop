...
function OrderSummary({ orderSummary }) {
  ...
  return (
    <div className={styles.orderSummaryWrapper}>
      ...
      <h3 className={styles.orderSummaryTotal}>{formatPrice(total)} gold</h3>

      <div className={styles.orderSummaryDetailWrapper}>
        <h3>
          Subtotal ({totalQty} {totalQty > 1 ? 'items' : 'item'})
        </h3>
        <span>{formatPrice(subtotal)} gold</span>
      </div>

      <div className={styles.orderSummaryDetailWrapper}>
        <h3>VAT (12%)</h3>
        <span>{formatPrice(vat)} gold</span>
      </div>
      ...
      <div className={`${styles.orderSummaryDetailWrapper} ${styles.orderSummaryTotal}`}>
        <h3>Total</h3>
        <span>{formatPrice(total)} gold</span>
      </div>
